...
This figure gives a broad overview of the components that we will play with
in this workshop. One of them, the _Kafka Streams Application_ will be our main
focus. It will read data from topics, apply transformations and output to new
topics. But you will also learn how to move data to and from Kafka with the help
of Kafka Connect.

| ![Architecture overview](search-app.png) | As you complete the exercises, the audiobook-search application will improve with each step. Most of the content will come from database tables, but also from _user like_ events generated by a different microservice.  |
| ------------- | ------------- |
